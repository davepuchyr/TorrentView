      ...
      <TableHead className={cn("whitespace-nowrap", className)}>
         <Button variant="ghost" onClick={e => onSort(columnKey, e.shiftKey)} className="-ml-4 h-8 px-2 sm:px-4">
            ...
            <div className="ml-2 flex items-center">
               <SortIcon className={cn("h-4 w-4", isSorted ? "text-accent" : "text-muted-foreground/50")} />
               {isSorted && sortConfig.length > 1 && (
                  <span className="ml-1 flex h-4 w-4 items-center justify-center rounded-full bg-muted text-xs font-normal text-muted-foreground">
                     {sortIndex + 1}
                  </span>
               )}
            </div>
                        ...
                        <ContextMenu key={torrent.hash}>
                           <ContextMenuTrigger asChild>
                              <TableRow
                                 ...
                                 onDoubleClick={() => handleDownload(torrent)}
                                 onAuxClick={e => {
                                    if (e.button === 1) {
                                       // Middle mouse button
                                       e.preventDefault();
